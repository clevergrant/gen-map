...
public class PersonsServiceTest {
	...
	public void getAll() throws Exception {
		...
		PersonsResult res = new PersonsService().getAll(
				new PersonsRequest(token.getToken())
		);

		Person[] fromDb = res.getData();

		assertEquals(fromDb[0], persons[0]);
		assertEquals(fromDb[1], persons[1]);
		assertEquals(fromDb[2], persons[2]);
	}
}